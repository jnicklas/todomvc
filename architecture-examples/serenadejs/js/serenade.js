...
format = function(model, key) {
  ...
  value = model[key];
  formatter = (_ref = model[key + "_property"]) != null ? _ref.format : void 0;
  if (typeof formatter === 'function') {
    return formatter.call(model, value);
  } else {
    return value;
  }
};
...
Event = (function() {
  ...
  function Event(object, name, options) {
    ...
    this.queue_name = "_s_" + name + "_queue";
    this.queue = this.object.hasOwnProperty(this.queue_name) ? this.object[this.queue_name] : this.object[this.queue_name] = [];
  ...
  Event.prototype.resolve = function() {
    ...
    return this.queue = this.object[this.queue_name] = [];
  };
        ...
        for (_j = 0, _len1 = _ref.length; _j < _len1; _j++) {
          ...
          if (type === "singular") {
            if (target === object[name]) {
              _results1.push((_ref2 = object[dependency + "_property"]) != null ? typeof _ref2.triggerChanges === "function" ? _ref2.triggerChanges(object) : void 0 : void 0);
            } else {
              _results1.push(void 0);
            }
          } else if (type === "collection") {
            if (__indexOf.call(object[name], target) >= 0) {
              _results1.push((_ref3 = object[dependency + "_property"]) != null ? typeof _ref3.triggerChanges === "function" ? _ref3.triggerChanges(object) : void 0 : void 0);
            } else {
              _results1.push(void 0);
            }
          } else {
...
SerenadeProperty = (function() {

  function SerenadeProperty(name, options) {
    var _i, _len, _ref;
    this.name = name;
    this.options = options;
    ...
    if (this.options.dependsOn) {
      _ref = [].concat(this.options.dependsOn);
      for (_i = 0, _len = _ref.length; _i < _len; _i++) {
        name = _ref[_i];
  ...
  SerenadeProperty.prototype.set = function(object, value) {
    if (typeof value === "function") {
      return this.options.get = value;
    } else {
      if (this.options.set) {
        this.options.set.call(object, value);
      } else {
        def(object, this.valueName, {
          value: value,
          configurable: true
        });
      }
      return this.triggerChanges(object);
    }
  };

  SerenadeProperty.prototype.get = function(object) {
    var listener, value,
      _this = this;
    ...
    if (this.options.get) {
      ...
      value = this.options.get.call(object);
      if (!("dependsOn" in this.options)) {
        object._s_property_access.unbind(listener);
      }
    } else {
      value = object[this.valueName];
    }
    object._s_property_access.trigger(this.name);
    return value;
  };

  SerenadeProperty.prototype.addDependency = function(name) {
  ...
  SerenadeProperty.prototype.triggerChanges = function(object) {
    var changes, name, value, _i, _len, _ref, _results;
    changes = {};
    _ref = [this.name].concat(this.dependents(object));
    for (_i = 0, _len = _ref.length; _i < _len; _i++) {
      name = _ref[_i];
      changes[name] = object[name];
    }
    object.change.trigger(changes);
    triggerGlobal(object, [this.name].concat(this.dependents(object)));
    _results = [];
    for (name in changes) {
      if (!__hasProp.call(changes, name)) continue;
      value = changes[name];
      _results.push(object["change_" + name].trigger(value));
    }
    return _results;
  };

  SerenadeProperty.prototype.registerGlobal = function(object) {
    var name, subname, type, _i, _len, _ref, _ref1, _results;
    if (!object["_glb_" + this.name]) {
      object["_glb_" + this.name] = true;
      _ref = this.globalDependencies;
      _results = [];
      for (_i = 0, _len = _ref.length; _i < _len; _i++) {
...
defineProperty = function(object, name, options) {
  ...
  async = "async" in options ? options.async : settings.async;
      ...
      get: function() {
        ...
        if (!this[valueName]) {
          ...
          this[valueName].change.bind(function() {
            return _this[name + "_property"].triggerChanges(_this);
          });
        }
...
Property = {
  style: function(ast, node, model, controller) {
    ...
    update();
    if (ast.bound) {
      return node.bindEvent(model["change_" + ast.value], update);
    }
  },
...
Compile = {
  ...
  text: function(ast, model, controller) {
    ...
    if (ast.bound) {
      node.bindEvent(model["change_" + ast.value], function() {
